...
-- Content goals drive both the Create-tab dropdown and the AI prompt (ai_guidance,
-- hashtags_*). The four builtins (is_builtin=true) have IDs load-bearing elsewhere —
-- 'booth_renters' in particular is matched literally by the renter-outcome UI and the
-- booth_renter_funnel()/booth_renter_performance() RPCs — so the app only ever lets
-- their id be immutable; everything else about a goal, builtin or not, is editable.
CREATE TABLE salon.content_goals (
  id TEXT PRIMARY KEY,
  label TEXT NOT NULL,
